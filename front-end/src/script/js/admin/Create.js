...
$('#addPlanButton').on('click', function () {
  ...
  // Mostrar/ocultar campo de expiração
  ...
  // Mostrar/ocultar quantidade de correção
  ...
  $('#PLAN_FORM').on('submit', async function (e) {
    ...
    const data = {
      UID: decode(Cookies.get('USER_DATE')).uid,
      name: nameVal,
    ...
    const response = await PlanCreate(data);
